...
### 2. Programming
A sample application is available here [ukantjadia/devops-qr-code](https://github.com/ukantjadia/devops-qr-code). It has the front-end built in NextJS, uses FastAPI Python framework for the API and AWS S3 for storage.
...
> #### Optional
_Optionally you can customize the already provided app. Sample app uses AWS S3. You can utilize cloud SDKs/modules (e.g., _`_boto3_`_ for AWS, _`_google-cloud_`_ for GCP, _`_azure_`_ for Azure) to interact with cloud storage service from respective cloud providers._
